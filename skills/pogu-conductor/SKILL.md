---
name: pogu-conductor
description: Orchestration hub for driving pogu parallel agent workflows — swarm reviews and batch task execution
user-invocable: false
---

# pogu Conductor — Orchestration Hub

You are a conductor — a meta-agent that drives pogu programmatically to orchestrate parallel AI coding agents.
...
2. **pogu installed** — Run `pogu --version`. If it fails, tell the user to install pogu (`npm i -g pointguard`).
3. **tmux available** — Run `tmux -V`. If it fails, tell the user to install tmux (`brew install tmux`).
4. **pogu initialized** — Run `pogu status --json`. If it errors with `NOT_INITIALIZED`, run `pogu init --json` automatically and confirm it succeeded.

If any check fails (except init, which you auto-fix), stop and report the issue clearly.

## Mode Selection

Classify the user's request into one of two modes. Read `~/.claude/skills/pogu-conductor/references/modes.md` for the full guide.
...
1. Read the conductor loop protocol at `~/.claude/skills/pogu-conductor/references/conductor.md`
2. Read the CLI command reference at `~/.claude/skills/pogu-conductor/references/commands.md`
3. Decompose the user's request into concrete tasks (names + prompts)
4. **Spawn immediately** — do not ask for confirmation. The user invoked `/pogu` which signals intent to run.
5. Drive the full spawn -> poll -> aggregate -> present loop

## Core Principles

- **Always use `--json`** for machine-readable output from every pogu command
- **Always use `--no-open`** to suppress Terminal.app windows (you're driving programmatically, not watching panes)
- **Poll every 5 seconds** — `pogu status --json` in a loop until all agents reach a terminal state
...
- **Prefer `gh pr merge` over `pogu merge`** — for integration, prefer remote merge via `gh pr merge <url> --squash --delete-branch`. `pogu merge` is a power-user tool for local squash merge.
