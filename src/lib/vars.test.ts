...
describe('parseVars', () => {
  ...
  test('given no = sign, should throw PpgError with INVALID_ARGS', () => {
    try {
      parseVars(['NOEQUALS']);
      ...
      expect((err as PpgError).message).toContain('Invalid --var format');
    }
  });

  test('given empty key (=value), should throw PpgError with INVALID_ARGS', () => {
    try {
      parseVars(['=value']);
      expect.fail('should have thrown');
    } catch (err) {
