...
pogu (pointguard) is a local orchestration system for parallel AI coding agents, driven through a native macOS dashboard. The dashboard is the primary interface — it lets you spawn, monitor, and merge agents visually. Underneath, a CLI engine manages git worktrees, tmux sessions, status tracking, and branch merging.
...
- Not a cloud service — pogu runs locally on your machine
- Not an IDE or editor — pogu is for agent orchestration, not code editing
- Not agent logic — pogu spawns and monitors agents, it doesn't implement agent behavior
- Not CI/CD — pogu is for interactive development, not automated pipelines
...
- **Local filesystem** — manifest, config, prompts, results, and templates all live in `.pogu/`
...
- **Worktree isolation model** — Git worktrees provide true filesystem isolation with shared history. Branch naming: `pogu/<name>`. Path: `.worktrees/wt-{id}/`
...
- **Manifest with file-level locking** — `proper-lockfile` (10s stale, 5 retries) + `write-file-atomic` for safe concurrent access from multiple pogu commands
...
- **Agent-agnostic config** — Agents defined in `.pogu/config.yaml` with `command`, `promptFlag`, `interactive` properties. Default: Claude Code
...
- `pogu init` to 3+ parallel agents running in < 2 minutes
