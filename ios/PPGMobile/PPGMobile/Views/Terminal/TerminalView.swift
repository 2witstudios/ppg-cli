    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            TerminalInputBar(text: $inputText) {
                guard !inputText.isEmpty, let ws = appState.wsManager else { return }
                ws.sendTerminalInput(agentId: agentId, text: inputText)
                inputText = ""
            }
        }
...
final class TerminalMessageRouter: @unchecked Sendable {
    ...
    func addSubscriber(
        ...
        if states[managerID] == nil {
            ...
            states[managerID] = State(previousOnMessage: previousOnMessage, subscribers: [:])
